...
2.  Open the project in pycharm then Run "pip install -r requirements.txt" within your pycharm or other IDE terminal to install all the necessary libraries needed to run the code.
    If running the requirements.txt file didn't install all necessary packages.  You may need to import additional packages.
    You can check what's needed by exploring the imports at the top of each used .py files to ensure that the libraries are installed.
    If not, pycharm will highlight it and provide an option to install the library.
...
7.  Go to main.py and update database credentials to reflect your local environment. (you might have to install the transformers library.  This can be done going to the tweet_analysis_transformers.py file.
    Also, you might have to install pytorch or tensorflow for the project to work.  'conda install pytorch torchvision torchaudio pytorch-cuda=11.8 -c pytorch -c nvidia' in the terminal.
